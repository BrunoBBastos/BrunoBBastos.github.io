class Enemy {
	...
	show() {
		...
		imageMode(CENTER);
		if (frameCount % 30 < 15) soldierImg[0] = soldierImg[1];
		else soldierImg[0] = soldierImg[2];
		image(soldierImg[0], 0, 0);
		pop();
	}
...
class Archer extends Enemy {
	...
	behavior() {
		...
		if (distance > this.sightRadius) {
			...
			let obj = player.pos.copy();
			this.direction = obj.sub(this.pos);
			archerImg[0] = archerImg[1];
			this.show();
		}
	}
