...
function mouseReleased() {
	switch (mode) {
		...
		case 1:
			if (levelSelButton.detectsMouse()) mode = 6;
			if (playButton.detectsMouse()) mode++;
			break;
		...
		case 6:
			if (lvl1Button.detectsMouse()) {
				mode = 1;
				level = 0;
			}
			if (lvl2Button.detectsMouse()) {
				mode = 1;
				level = 1;
			}
			if (lvl3Button.detectsMouse()) {
				mode = 1;
				level = 2;
			}
			if (lvl4Button.detectsMouse()) {
				mode = 1;
				level = 3;
			}
			if (lvl5Button.detectsMouse()) {
				mode = 1;
				level = 4;
			}
			break;
...
function detectCollisions() {
	...
	for (let a = 0; a < arrows.length; a++) {
		...
		for (let e = 0; e < enemies.length; e++) {
			if (circleSqCollision(arrows[a], enemies[e])) {
				enemies[e].life--;
				if (enemies[e].life <= 0) {
					enemies[e].die();
					enemies.splice(e, 1);
	...
	for (let e = 0; e < enemies.length; e++) {
		if (circleSqCollision(player, enemies[e])) {
			levelResult = false;
			mode = 5;
			break;
		}
...
function circleSqCollision(c, r) {
